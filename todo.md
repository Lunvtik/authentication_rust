...
- [ ] validate_credentials: single entry checks user and password
- [x] spawn_blocking so hashing does not freeze the async server
- [ ] dummy-hash fallback against timing leak for unknown user
- [ ] same error message for unknown user and wrong password
- [x] AuthError type
- [ ] tests: valid, invalid, unknown user
...
## UserStore
- [ ] sqlx and Postgres connection pool in common/db.rs
- [ ] migration: users table (id, email unique, password_hash)
- [ ] index on email for the login lookup
